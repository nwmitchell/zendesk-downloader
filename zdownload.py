...
def get_updated_tickets(user, password, start_time):
	ticket_list = []
	url = "https://cumulusnetworks.zendesk.com/api/v2/incremental/tickets.json?start_time={0}".format(start_time.strftime("%s"))
...
def get_case_info(user, password, ticket_id):
	case_info = {}
	url = "https://cumulusnetworks.zendesk.com/api/v2/tickets/{0}.json".format(ticket_id)
	result = requests.get(url, auth=(user, password)).json()
	if "error" in result:
		...
		if case_info['org_id'] != "None":
			url = "https://cumulusnetworks.zendesk.com/api/v2/organizations/{0}.json".format(case_info['org_id'])
			result = requests.get(url, auth=(user, password)).json()
			if "error" in result:
...
def get_attachment_list(user, password, ticket_id):
	attachment_list = []
	url = "https://cumulusnetworks.zendesk.com/api/v2/tickets/{0}/comments.json".format(ticket_id)
	result = requests.get(url, auth=(user, password)).json()
...
def download_attachments(user, password, ticket_id, directory):
	print "CS#{0}".format(ticket_id)
	attachment_list = get_attachment_list(user, password, ticket_id)
	if attachment_list:
...
if __name__ == "__main__":
	...
	if '{0}'.format(arguments['--case']) == 'None':
		...
		updated_tickets =  get_updated_tickets(user, password, start_time)
		for ticket in updated_tickets:
			case_info = get_case_info(user, password, ticket)
			if not "error" in case_info:
				case_dir = "{0}{1}_{2}_{3}".format(download_directory, case_info['id'], case_info['org_name'], case_info['org_id'])
				download_attachments(user, password, ticket, case_dir)
			else:
